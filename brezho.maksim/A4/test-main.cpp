...
{
  brezho::Circle circle({ 1.0, 4.0 }, 1.0);
...
{
  brezho::point_t array1[] = { { 1.0, 0.0 }, { 0.0, 3.0 }, { 4.0, 6.0 }, { 5.0, 3.0 }, { 3.0, 0.0 } };
  brezho::Polygon polygon(array1, 5);
...
{
  brezho::point_t array1[] = { { 1.0, 0.0 }, { 3.0, 2.0 }, { 4.0, 6.0 }, { 5.0, 3.0 }, { 3.0, 0.0 } };
  brezho::point_t array2[] = { { 1.0, 0.0 }, { 2.5, 3.0 }, { 4.0, 6.0 }, { 5.0, 3.0 }, { 3.0, 0.0 } };
  brezho::point_t array3[] = { { 0.0, 0.0 }, { 4.0, 5.0 }, { 0.0, 5.0 }, { 4.0, 0.0 } };
  BOOST_CHECK_THROW(brezho::Polygon polygon(array1, 5), std::invalid_argument);
  BOOST_CHECK_THROW(brezho::Polygon polygon(array2, 5), std::invalid_argument);
  BOOST_CHECK_THROW(brezho::Polygon polygon(array3, 4), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(test_permanency_size_polygon_moving)
{
  ...
  brezho::rectangle_t frameRect = polygon.getFrameRect();
...
{
  brezho::point_t array1[] = { { 1.0, 0.0 }, { 0.0, 3.0 } };
  BOOST_CHECK_THROW(brezho::Polygon polygon(array1, 2), std::out_of_range);
}
...
{
  brezho::CompositeShape array1;
  std::shared_ptr<brezho::Shape> rect = std::make_shared<brezho::Rectangle>(brezho::Rectangle({ 7.0, 2.0 }, 2.0, 2.0));
  std::shared_ptr<brezho::Shape> triangle = std::make_shared<brezho::Triangle>(brezho::Triangle({ 4.0, 0.0 }, { 0.0, 0.0 }, { 0.0, 3.0 }));
  std::shared_ptr<brezho::Shape> circle = std::make_shared<brezho::Circle>(brezho::Circle({ 1.0, 5.0 }, 1.0));
  array1.addShape(rect);
  array1.addShape(triangle);
  array1.addShape(circle);
  array1.rotate(90);
  brezho::rectangle_t frameRect = array1.getFrameRect();
...
{
  std::shared_ptr<brezho::Shape> rect = std::make_shared<brezho::Rectangle>(brezho::Rectangle({ 1.0, 2.0 }, 2.0, 2.0));
  std::shared_ptr<brezho::Shape> triangle = std::make_shared<brezho::Triangle>(brezho::Triangle({ 4.0, 0.0 }, { 0.0, 0.0 }, { 0.0, 3.0 }));
  std::shared_ptr<brezho::Shape> circle = std::make_shared<brezho::Circle>(brezho::Circle({ 1.0, 5.0 }, 1.0));
  brezho::CompositeShape compositeShape;
  compositeShape.addShape(rect);
  compositeShape.addShape(triangle);
  compositeShape.addShape(circle);
  brezho::Matrix matrix = compositeShape.makeMatrix();
