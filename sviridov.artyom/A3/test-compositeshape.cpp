...
{
  std::shared_ptr<sviridov::Shape>shape = nullptr;
  BOOST_CHECK_THROW(sviridov::CompositeShape comp(shape), std::invalid_argument);
}

BOOST_AUTO_TEST_CASE(constructor_comp_shape_rectangle)
{
  auto rect = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 0, 0 }, 100, 10);
  sviridov::CompositeShape comp(rect);
  BOOST_CHECK(rect->getFrameRect() == comp.getFrameRect());
}

BOOST_AUTO_TEST_CASE(constructor_comp_shape_circle)
{
  auto cir = std::make_shared<sviridov::Circle>(sviridov::point_t{ 10, -10 }, 11);
  sviridov::CompositeShape comp(cir);
  BOOST_CHECK(cir->getFrameRect() == comp.getFrameRect());
}

BOOST_AUTO_TEST_CASE(constructor_comp_shape_tringle)
{
  auto tri = std::make_shared<sviridov::Triangle>(sviridov::point_t{ 0, 0 },
      sviridov::point_t{ 10, 8 }, sviridov::point_t{ -9, 4 });
  sviridov::CompositeShape comp(tri);
  BOOST_CHECK(tri->getFrameRect() == comp.getFrameRect());
}

BOOST_AUTO_TEST_CASE(compositeshape_rectangle)
{
  auto rect = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 11, 13 }, 10, 1);
  sviridov::CompositeShape comp(rect);
...
{
  auto cir = std::make_shared<sviridov::Circle>(sviridov::point_t{ 11, 13 }, 10);
  sviridov::CompositeShape comp(cir);
...
{
  auto rect_one = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 11, 13 }, 10, 1);
  sviridov::CompositeShape comp(rect_one);
  auto rect_two = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 1, 1 }, 10, 1);
...
{
  auto cir_one = std::make_shared<sviridov::Circle>(sviridov::point_t{ 11, 13 }, 10);
  sviridov::CompositeShape comp(cir_one);
  auto cir_two = std::make_shared<sviridov::Circle>(sviridov::point_t{ 1, 1 }, 10);
...
{
  auto tri_one = std::make_shared<sviridov::Triangle>(sviridov::point_t{ 0, 0 },
      sviridov::point_t{ 10, 8 }, sviridov::point_t{ -9, 4 });
  sviridov::CompositeShape comp(tri_one);
  auto tri_two = std::make_shared<sviridov::Triangle>(sviridov::point_t{ 0, 0 },
      sviridov::point_t{ 10, 8 }, sviridov::point_t{ -9, 4 });
...
{
  auto rect_one = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 0,0 }, 2, 2);
  sviridov::CompositeShape comp(rect_one);
  auto rect_two = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 2,0 }, 2, 4);
  comp.addShape(rect_two);
  sviridov::rectangle_t frame_rect = { { 1,0 },4,4 };
  BOOST_CHECK(comp.getFrameRect() == frame_rect);
}

BOOST_AUTO_TEST_CASE(add_compositeshape_circle_frame_rect)
{
  auto cir_one = std::make_shared<sviridov::Circle>(sviridov::point_t{ 0,0 }, 2);
  sviridov::CompositeShape comp(cir_one);
  auto cir_two = std::make_shared<sviridov::Circle>(sviridov::point_t{ 2,0 }, 2);
  comp.addShape(cir_two);
  sviridov::rectangle_t frame_rect = { { 1,0 },6,4 };
  BOOST_CHECK(comp.getFrameRect() == frame_rect);
}
...
{
  auto tri_one = std::make_shared<sviridov::Triangle>(sviridov::point_t{ 0,0 },
      sviridov::point_t{ 1,1 }, sviridov::point_t{ 6,0 });
  sviridov::CompositeShape comp(tri_one);
  auto tri_two = std::make_shared<sviridov::Triangle>(sviridov::point_t{ 0,0 },
      sviridov::point_t{ 1,1 }, sviridov::point_t{ 0,6 });
  comp.addShape(tri_two);
  sviridov::rectangle_t frame_rect = { { 3,3 },6,6 };
  BOOST_CHECK(comp.getFrameRect() == frame_rect);
}

BOOST_AUTO_TEST_CASE(area_compositeshape)
{
  auto rect = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 1,1 }, 10, 5);
  auto cir = std::make_shared<sviridov::Circle>(sviridov::point_t{ -5,3 }, 4);
  auto tri = std::make_shared<sviridov::Triangle>(sviridov::point_t{ 10, 2 },
      sviridov::point_t{ -3, 2 }, sviridov::point_t{ -2, -2 });
  sviridov::CompositeShape comp(rect);
  comp.addShape(cir);
  comp.addShape(tri);
...
BOOST_AUTO_TEST_CASE(move_absolutely_compositeshape)
{
  ...
  comp.addShape(cir);
  comp.addShape(tri);
  sviridov::rectangle_t frame_rect = comp.getFrameRect();
  comp.move({ 100, -100 });
...
{
  auto rect1 = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 1,1 }, 2, 4);
  auto rect2 = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 2,2 }, 2, 2);
  sviridov::CompositeShape comp(rect1);
  comp.addShape(rect2);
  rect2->move({ 3,3 });
  sviridov::rectangle_t frame_rect = { { 2,1.5 },4,5 };
  BOOST_CHECK(comp.getFrameRect() == frame_rect);
}

BOOST_AUTO_TEST_CASE(relation_circle)
{
  auto rect = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 1,1 }, 2, 4);
  auto cir = std::make_shared<sviridov::Circle>(sviridov::point_t{ 2,2 }, 1);
  sviridov::CompositeShape comp(rect);
  comp.addShape(cir);
  cir->move({ 3,3 });
  sviridov::rectangle_t frame_rect = { { 2,1.5 },4,5 };
...
{
  auto rect = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 1,1 }, 2, 4);
  auto tri = std::make_shared<sviridov::Triangle>(sviridov::point_t{ 3,3 },
      sviridov::point_t{ 5,3 },sviridov::point_t { 5,1 });
  sviridov::CompositeShape comp(rect);
  comp.addShape(tri);
  tri->move(1, 0);
  sviridov::rectangle_t frame_rect = { { 3,1 },6,4 };
  BOOST_CHECK(comp.getFrameRect() == frame_rect);
}
...
{
  auto rect = std::make_shared<sviridov::Rectangle>(sviridov::point_t{ 1,1 }, 2, 4);
  sviridov::CompositeShape comp1(rect);
  sviridov::CompositeShape comp2(rect);
