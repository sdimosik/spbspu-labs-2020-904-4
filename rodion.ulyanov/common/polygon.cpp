...
namespace rodion {
  ...
  void Polygon::rotate(double angle) noexcept
  {
    ...
    {
      tempX = points_[i].x;
      points_[i].x = (points_[i].x - pos.x) * std::cos(angle_) - (points_[i].y - pos.y) * std::sin(angle_) + pos.x;
      points_[i].y = (tempX - pos.x) * std::sin(angle_) + (points_[i].y - pos.y) * std::cos(angle_) + pos.y;
    }
  }
  ...
  {
    std::cout << "Polygon has an angle of: " << angle_ << '\n';
  }
}
