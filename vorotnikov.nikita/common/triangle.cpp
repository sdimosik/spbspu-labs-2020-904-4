...
namespace vorotnikov {
  ...
  double Triangle::getArea() const {
    double half_perimetr = (point_one_.getLength(point_two_) + point_two_.getLength(point_three_)
        + point_one_.getLength(point_three_)) / 2;
    return sqrt(half_perimetr * (half_perimetr - point_one_.getLength(point_two_))
        * (half_perimetr - point_two_.getLength(point_three_))
          * (half_perimetr - point_one_.getLength(point_three_)));
  }

  rectangle_t Triangle::getFrameRect() const {
    double left = std::min({point_one_.x, point_two_.x, point_three_.x});
    double right = std::max({point_one_.x, point_two_.x, point_three_.x});
    double low = std::min({point_one_.y, point_two_.y, point_three_.y});
    double high = std::max({point_one_.y, point_two_.y, point_three_.y});
  ...
  void Triangle::scale(double value) {
    ...
    point_one_.x -= (value - 1) * (pos_.x - point_one_.x);
    point_two_.x -= (value - 1) * (pos_.x - point_two_.x);
    point_three_.x -= (value - 1) * (pos_.x - point_three_.x);
