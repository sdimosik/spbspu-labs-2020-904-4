...
vorotnikov::Matrix::Matrix(const vorotnikov::Matrix &other) :
...
vorotnikov::Matrix::Matrix(vorotnikov::Matrix &&other) noexcept :
...
vorotnikov::Matrix &vorotnikov::Matrix::operator=(const vorotnikov::Matrix &other)
{
  if (this == &other)
  {
    return *this;
...
std::shared_ptr<vorotnikov::Shape> &vorotnikov::Matrix::getShape(size_t row, size_t column)
{
  if (row >= rows_)
  {
...
void vorotnikov::Matrix::addShape(const std::shared_ptr<Shape> &shape)
{
  if (!shape)
  {
...
bool vorotnikov::Matrix::isOverlapping(const std::shared_ptr<Shape> &first, const std::shared_ptr<Shape> &second) const {
  if (!first || !second) {
