...
namespace vorotnikov
{
  ...
  {
    if (capacity_ <= 0) {
      throw std::invalid_argument("Composite Shape error: invalid argument: capacity must be positive!");
    }
  }

  CompositeShape::CompositeShape(const vorotnikov::CompositeShape &other) :
  ...
  std::shared_ptr<Shape> &CompositeShape::operator[](const size_t index) const {
    if (index >= size_) {
      throw std::out_of_range("Composite shape error: out of range: index must be >= 0 and < shape size!");
    }
  ...
  void CompositeShape::scale(const double factor) {
    if (factor <= 0) {
      throw std::invalid_argument("Composite shape error: invalid argument: scale value must be positive.");
    }
