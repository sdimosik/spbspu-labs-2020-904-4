...
namespace bulanov
{
  ...
  Matrix::shapePtr &Matrix::Layer::operator[](size_t index)
  {
    ...
    {
      throw std::out_of_range("Invalid index of column received. False index: " + std::to_string(index));
    }
  ...
  Matrix &Matrix::operator=(const Matrix &element)
  {
    ...
    {
      matrixFigure temp(new shapePtr[element.levels_ * element.columns_]);
  ...
  void Matrix::addElement(const Matrix::shapePtr &shape)
  {
    ...
    if (levels_ == 0)
    {
      matrixFigure newElements = std::make_unique<shapePtr[]>(1);
      columns_++;
      levels_++;
