...
int main()
{
  try
  {
    ...
    matrix.getSizeMatrix();
    vasilevskaya::Shape::PointToShape circleForMatrix
      = std::make_shared<Circle>(vasilevskaya::point_t{ -10, -10 }, 1);
    matrix.addShape(circleForMatrix);
    std::cout << "SIZE OF MATRIX : " << matrix.getSizeMatrix() << '\n';
    matrix.printMatrix();
